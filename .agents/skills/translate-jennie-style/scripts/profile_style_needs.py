...
GOALS = {
    ...
    "defined_waist": "用衣摆、扣合点、腰带或短外套把用户选择的腰位做清楚；腰线明确不等于必须抬到自然腰。",
...
def normalize(data: dict) -> dict:
    ...
    waist_notes = {
        ...
        "below_natural_waist": "成衣腰线常偏低：用腰带、短上衣或敞开外套把低位交界做得清楚；只有主动想换效果时才比较更高腰位。",
        "unsure": "连衣裙腰线位置暂不确定：可分别比较低腰与自然腰交界，不据此下身材结论，也不覆盖已表达的低腰偏好。",
